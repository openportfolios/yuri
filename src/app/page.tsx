...
import { hasItems, portfolioConfig, px, sectionOrder, sectionTitle, type OrderableSectionKey } from "@/lib/portfolio-config";
...
function WorkExperienceSection() {
  if (!hasItems(WORK_EXPERIENCE)) return null;
  return (
    <Reveal>
    <section className="flex min-h-0 flex-col gap-y-3 print:gap-y-1">
...
function BlogSection({ posts }: { posts: ReturnType<typeof getAllPosts> }) {
...
export default function Home() {
  ...
  // Sections render in the order their keys appear in portfolio.config.json.
  ...
  const sections: Record<OrderableSectionKey, React.ReactNode> = {
    ...
    blog: <BlogSection posts={blogPosts} />,
    discordActivity: (
      <Reveal>
